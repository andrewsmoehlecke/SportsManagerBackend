...
public class UsuarioTimeController {
    ...
    @GetMapping()
    public ResponseEntity<List<UsuarioTime>> getAll() {
        log.info("GET /usuario_time");

        List<UsuarioTime> allUsuarioTime = usuarioTimeDao.findAll();
        return new ResponseEntity<>(allUsuarioTime, HttpStatus.OK);
    }

    @GetMapping("/{id}")
    public ResponseEntity<UsuarioTime> getUsuarioTimeById(@PathVariable("id") long id) {
